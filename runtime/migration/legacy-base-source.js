...
function notificationTarget(row) {
  ...
  return {
    ...
    bot_id: 'legacy-exact-base', chat_type: 'dm', chat_id: row.endpoint_id,
...
function pendingEnvelope(row, batchId, recordId, sequence, observedAt) {
  return {
    ...
    chat_type: 'dm', chat_id: row.endpoint_id,
    native_thread_or_topic_id: null,
  ...
  return database.transaction(() => {
    ...
    for (const row of conversations) {
      ...
      if (row.direction === 'in' && state === 'pending') {
        sequence += 1;
        const route = typeof row.endpoint_id === 'string' && row.endpoint_id.length > 0
          ? 'unique' : 'ambiguous';
        records.push({
          kind: 'c4', legacy_record_id: recordId, legacy_state: 'pending', route,
          ...(route === 'unique' ? {
            legacy_queue_sequence: sequence,
            envelope: pendingEnvelope(row, batchId, recordId, sequence, observedAt),
          } : {}),
        });
  ...
  return database.transaction(() => {
    ...
    if (tableExists(database, 'conversations')) {
      ...
      for (const record of rollbackBatch.records) {
        if (record.kind !== 'c4' || !record.legacy_record_id.startsWith('conversation:')) continue;
...
export function legacyBatchDigest(batch) {
